...
&emsp;**这里有   微信小程序开发中遇到的所有坑，以及在填坑过程中的一些个人经验。  利用这篇教程存储一些常用的微信小程序开发技巧，方便查找。它可能教不了你什么，但至少能省下你百度的功夫。**  
...
* 第二， ** ** 的文笔并没有想象中的那么好，想想如果我带你们走了一遍小程序开发，然后你们以为是一条平坦路，结果碰到一堆坑坑洼洼，咋办？最后的锅，会不会到我背啊，可怕！  

&emsp;所以，在这里， ** ** 结合 **“日常躺坑”** ，先为你解决小程序的 100 个坑！虽然现在可能还不够，但是第一天我就碰到 4/5 个了，我想我可以帮你们躺完 100 个的！！！  
...
&emsp;关于轮播图的地址跳转，在微信小程序的官网是没用提及的，也是 ** ** 去百度查看了下，才知道怎么设置（可能是我一开始就挑战的难度太高了么 -_-|| ），在下面 ** ** 贴出来代码~想知道怎么解决的可以去看看：首先，在 `data` 中设置 `link` ；然后，设置 `navigator` 导航遍历 `item.link` 。
...
&emsp;关于 `wx:key` ， `wx:key` 的作用是：当数据改变触发渲染层重新渲染的时候，会校正带有 key 的组件，框架会确保他们被重新排序，而不是重新创建，以确保使组件保持自身的状态，并且提高列表渲染时的效率。但是，在其 `swiper` 中，小程序本身是没有写的，所以它会带有 `warning` ，这里也是个小坑， ** ** 也是百度了下也知道这件事：[点我了解](https://www.sohu.com/a/207728111_99897596)。
...
&emsp;Flex布局又称弹性布局，在小程序开发中比较适用。但是由于 ** ** 之前没怎么用过 Flex 布局，所以在这里咱们特意去踩下坑，充实下自己。[【小程序开发之页面布局】](https://blog.csdn.net/anda0109/article/details/72867449)[【阮一峰-Flex 布局教程】](http://www.techug.com/post/flex-examples.html)  
...
&emsp;你注意到了吗？在 `*.wxml` 中， ** ** 设置了个空的 `<view>` ，如果你把这个 `<view>` 去掉，你会惊奇地发现，它……下来了……
...
&emsp;是的，** ** 打死都不承认这是我自己的锅，新手注意！新手注意！！新手注意！！！
...
&emsp;需求是不是很简单~既然微信小程序有自己的官方文档。那么，怎么方便怎么来吧，于是 ** ** 在微信小程序中搜索关键字 `弹窗`：
...
&emsp;Duang~~~这不就是我要的效果么，挖槽，黑科技？于是 ** ** 去小程序那里搜了下 `modal` ……enm...蜜汁尴尬，好像只有上面的 `wx.showModal()` 方法……于是 ** ** 满头黑线……好嘛，黑科技黑科技！！！  
&emsp;下面贴出实现代码：
...
&emsp;综上，** ** 气得差口吐白沫了……换换换！有空要换成其他两种方式才行！！！
...
&emsp;** ** 还未使用过 rich-text，这里先预留个坑。如果小伙伴们在开发 rich-text 过程中碰到过各种坑，可以跟 ** ** 提一下，我会写进这章节，顺带在章节尾写上你的大名，辛苦了~
...
&emsp;** ** 还未使用过 web-view，这里先预留个坑。如果小伙伴们在开发 web-view 过程中碰到过各种坑，可以跟 ** ** 提一下，我会写进这章节，顺带在章节尾写上你的大名，辛苦了~
...
&emsp;答案是，有的：[地址](https://developers.weixin.qq.com/miniprogram/dev/api/wxml/NodesRef.html)。但是，在这里， ** ** 并不推荐小伙伴这么做，想要学好一门新的技术，就不能因为旧的技术而限制了自己的观念。时代在进步，科技在发展，我们不学更多的知识，只能被淘汰在前端的潮流中。
...
&emsp;我们都知道，在微信小程序中，有个 `onLaunch` 方法，微信小程序官方文档对其描述就是：每个页面进来需要先加载 `onLaunch` 方法，再去执行其他方法。然后，在 ** ** 尝试设置在 `onLaunch` 中调用 `wx.login()`，却发现，`index.js` 的 `onLoad` 方法是先于 `onLaunch` 执行的，这导致我们没法预先获取到需要的信息：    

1. 先执行 `index.js` 的 `onload`
2. 再才执行 `app.js` 中的 `onLaunch`

&emsp;迫于无奈，** ** 对其进行了百度：[地址](https://blog.csdn.net/qq_35860064/article/details/82590573) 。通过百度这篇文章发现，有两个解决方案：

1. 设置启动页，成功再返回首页
2. 使用 `Promise` 来进行进程管理

&emsp;但是，由于 ** ** 对于 `Promise` 的做法，觉得其太过复杂，故新增了一个 `page/login`。
...
&emsp;由于 ** ** 平时都是使用 ES5 ，一直口嚷嚷说要步入 ES6 大堂，但是一直就是没入门，所以乘着有功夫，尽量使用 ES6 ，用熟了才能进步。
...
&emsp;在项目的开发中，** ** 碰到了联系人列表的开发需求。话不多说，直接上手：
...
2. 编写 `Wxss` 定位，** ** 对 `Wxml` 进行了 `position: fixed` 的绝对定位，这样就可以将它固定在右侧。
